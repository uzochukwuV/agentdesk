...
function renderDecisionReceipt(p, compact = false) {
  if (!p) return "";
  const mid = p.bookYes?.mid;
  const edge = mid == null ? null : Number(p.probUp) - Number(mid);
  const features = Object.entries(p.features || {})
    .filter(([, value]) => Number.isFinite(Number(value)))
    .slice(0, compact ? 3 : 6);
  return `<details class="decision-receipt"${compact ? "" : " open"}><summary><span>DECISION RECEIPT</span><b>${esc(p.direction)} · ${fmt(p.confidence * 100, 0)}% confidence</b></summary><div class="receipt-grid"><span>Model P(UP)<b>${fmt(p.probUp, 3)}</b></span><span>YES mid<b>${fmt(mid, 3)}</b></span><span>Model edge<b class="${edge != null && edge >= 0 ? "pnl-pos" : "pnl-neg"}">${edge == null ? "–" : `${edge >= 0 ? "+" : ""}${fmt(edge * 100, 1)} pts`}</b></span><span>Signal price<b>${money(p.sourcePriceAtSignal)}</b></span></div><p class="receipt-rationale">${esc(p.rationale)}</p>${features.length ? `<div class="receipt-features">${features.map(([key, value]) => `<span>${esc(key)} <b>${fmt(value, 3)}</b></span>`).join("")}</div>` : ""}<div class="receipt-footer"><span>${p.decision === "trade" ? "Trade-qualified signal" : "Published signal · no execution"}</span><span>${p.createdAt ? `${ago(p.createdAt)} ago` : ""}</span></div></details>`;
}
...
async function updateTradeSummary() {
  ...
  try {
    ...
    const riskClass = risk.label === "HEALTHY" ? "healthy" : risk.label === "CAUTION" ? "caution" : "thin";
    summary.innerHTML = `<div class="quote-grid"><span>Average entry<b>${fmt(quote.avgPrice, 3)}</b></span><span>Estimated max loss<b>${fmt(risk.maxLossCollateral ?? quote.costCollateral, 3)} tUSDC</b></span><span>Quoted size<b>${fmt(quote.quotedContracts, 2)} contracts</b></span><span>Expected fill<b>${fmt(quote.filledContracts, 2)} contracts</b></span><span>Book levels<b>${quote.levelsConsumed}</b></span><span>Slippage vs mid<b>${fmt(Math.abs(quote.slippageVsMid) * 100, 2)} pts</b></span></div><div class="execution-verdict ${riskClass}"><div><small>EXECUTION SAFETY</small><b>${esc(risk.label || "UNASSESSED")} · ${fmt(risk.score, 0)}/100</b></div><span>${risk.warnings?.length ? esc(risk.warnings.join(" · ")) : "Two-sided book, acceptable impact, and enough time to expiry."}</span></div>${partial ? `<div class="quote-warning">Thin book: ${fmt(quote.wouldRestContracts, 2)} contracts would not fill and will be cancelled by IOC.</div>` : ""}<div class="quote-note">Safety score uses live fill ratio, spread, price impact, and seconds to expiry. It is an execution check, not a profit forecast. Quote refreshed at ${quote.fetchedAt ? new Date(quote.fetchedAt).toLocaleTimeString() : "now"} and can change before confirmation.</div>`;
  } catch (error) {
    if (requestId !== previewSeq) return;
...
function renderStrategyPicker() {
  return `<div class="strategy-picker">${LEADER.map((s) => `<div class="strategy-option"><div><span class="dot" style="background:${s.color}"></span><strong>${esc(s.name)}</strong><small>${esc(s.style)}</small></div>${followButton(s.id)}</div>`).join("")}</div>`;
}
...
async function renderAgentDetail(id) {
  ...
  $("#page-content").innerHTML = `<div class="detail-hero"><div><a class="back" href="#/app/agents">← All agents</a><h2><span class="dot" style="background:${d.color}"></span>${esc(d.name)}</h2><p>${esc(d.description)}</p></div><div class="detail-actions">${followButton(id)}${pred ? `<button class="trade-button" data-trade="${esc(active.window.marketId)}" data-symbol="${esc(active.window.symbol)}" data-side="${pred.direction === "DOWN" ? "NO" : "YES"}" data-agent="${esc(d.name)}">Copy current call →</button>` : ""}</div></div>
    <div class="stats-grid">${[[d.trades,"trades"],[`${d.won}-${d.lost}-${d.void}`,"won · lost · void"],[pct(winRate),"win rate"],[pct(d.accuracyPct),"prediction accuracy"],[fmt(d.stake,2),"staked"],[d.roiPct == null ? "–" : pct(d.roiPct),"ROI"],[fmt(d.pnl,2),"settled PnL"],[d.predictions,"predictions"]].map(([v,l]) => `<div class="stat-card"><b>${v}</b><span>${l}</span></div>`).join("")}</div>
     <div class="content-grid"><section class="panel"><div class="panel-heading"><div><span class="section-kicker">PREDICTIONS</span><h2>How ${esc(d.name)} thinks</h2></div><span class="heading-note">Newest first</span></div><div class="feed">${(d.recentPredictions || []).slice(0, 12).map((p) => `<div class="pred"><div class="row1"><span class="dir ${p.direction}">${p.direction} on <b>${esc(p.symbol)}</b></span>${dirTag(p)}</div>${renderDecisionReceipt(p)}</div>`).join("") || '<div class="empty-state">No predictions published yet.</div>'}</div></section><section class="panel"><div class="panel-heading"><div><span class="section-kicker">EXECUTIONS</span><h2>Public trade history</h2></div></div><div class="trades">${renderAgentTrades(d.recentTrades || [])}</div></section></div>
